...
Generates planner_combined.ics for Aug 2-31, 2026 based on data/all_days.json and the routine. """ from datetime import date, datetime, time, timedelta import json import uuid

DATA_PATH = "data/all_days.json"
...
with open(DATA_PATH,'r',encoding='utf-8') as f: meta = json.load(f)

now = datetime.utcnow() ical_events = []

current = START while current <= END: ds = current.isoformat() daymeta = meta.get(ds, {}) focus = daymeta.get('focus','General study') resources = daymeta.get('resources','')
...
# Study events
study_slots = [ ("Study 1", time(9,0), 90), ("Study 2", time(10,45), 90), ("Study 3", time(13,0), 90), ("Flashcards / Review", time(14,45), 60) ]
...
uid = str(uuid.uuid4())
checklist = 'Daily Completion Checklist\n' + '\n'.join([f'☐ {g}' for g in ['Complete lessons','30 practice questions','Review mistakes','20 minutes flashcards','Drink water and stretch during breaks']])
...
ical = ['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//TEAS Planner Full//EN'] ical.extend(ical_events) ical.append('END:VCALENDAR')

with open('planner_combined.ics','w',encoding='utf-8') as f: f.write('\r\n'.join(ical) + '\r\n')
